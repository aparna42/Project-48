...
function preload() {
    ...
    //Scaling is a sprite's property
    //Try using image() instead for the background if you want to assign a different size
    // "title1.jpg".scale = 1.45
    sound = loadSound("cannon.wav")
    rocK = loadSound("cannon.wav")
    backGround = loadSound("bmusic.wav")
}
...
function setup() {
    ...
    if (gameState === "start") {
        // Changed image to test
        titleButton = createImg("heart.png");
...
function draw() {
    background(title)
    //Hide the titleButton only in the play state
